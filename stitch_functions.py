...
def warp(pairs):
    srcTri = np.array( [[pairs[0,0], pairs[0,1]], [pairs[1,0], pairs[1,1]], [pairs[2,0], pairs[2,1]]] ).astype(np.float32)
    dstTri = np.array( [[pairs[0,2], pairs[0,3]], [pairs[1,2], pairs[1,3]], [pairs[2,2], pairs[2,3]]] ).astype(np.float32)
...
def ransac(matches, threshold, iters):
    ...
    for i in range(iters):
        points = random_point(matches, k=50)
...
#
def ransac_warp(matches, threshold, iters):
    num_best_inliers = 0

    for i in range(iters):

        points = random_point(matches, k=3)
        W = warp(points)
        H = np.concatenate((W, [[0,0,1]]), axis=0)
        #  avoid dividing by zero
        if np.linalg.matrix_rank(H) < 3:
            continue
        errors = get_error(matches, H)
...
def find_homo(img1, img2):
    ...
    matches = matcher(kp_left, des_left, left_rgb, kp_right, des_right, right_rgb, 0.6)####################### threshold=0.5
    #print(f"matches number = {len(matches)}")
    inliers, H = ransac(matches, 0.5, 2000)#################################################################### threshold=0.5
    return inliers, H

def find_homo_on_resized(img1, img2):
    left_gray, left_origin, left_rgb = read_image(img1)
    right_gray, right_origin, right_rgb = read_image(img2)
    left_gray = cv2.resize(left_gray, (left_gray.shape[1]//2,left_gray.shape[0]//2))
    right_gray = cv2.resize(right_gray, (right_gray.shape[1]//2,right_gray.shape[0]//2))
    left_rgb = cv2.resize(left_rgb, (left_rgb.shape[1]//2,left_rgb.shape[0]//2))
    right_rgb = cv2.resize(right_rgb, (right_rgb.shape[1]//2,right_rgb.shape[0]//2))
    kp_left, des_left = SIFT(left_gray)
    kp_right, des_right = SIFT(right_gray)
    ...
    r_start = time.time()
    inliers, H = ransac(matches, 0.5, 2000)
    #print(f"ransac time = {time.time() - r_start}")
